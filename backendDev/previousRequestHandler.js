...
function registerBetFunction(request,response){
    ...
    if(apostadorID && role=='apostador'){
        ...
        console.log(`O tamanho dos eventos é --> ${request.body.Eventos.length}`)
        dbComms.registerBetOnDb(request.body.Aposta,request.body.Eventos,request.body.Codigo,function(result){
            //var data=JSON.parse(JSON.stringify(request.body.Eventos))
